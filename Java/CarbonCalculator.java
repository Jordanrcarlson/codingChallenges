...
public class CarbonCalculator {
    ...
    private static final double KGCO2_PER_LGAS       = 12.85;   // KG of CO2 per litre of gas
    private static final double KGCO2_PER_M3GAS      = 1.94;    // KG of CO2 per cubic meter of gas
    private static final double M3_PER_GJGAS         = 26.137;  // cubic meters (M3) of gas per gigajoule (GJ)
    private static final double GJ_MANUF_COST        = 120;     // total auto manufacturing cost in GJs
    private static final double GJ_MAINT_COST_PER_YR = 3.8;     // auto maintenance cost per year in GJs
    private static final double KGCO2_PER_TRANSITKM  = 0.18;    // transit cost per km in KG of CO2
    private static final double KGS_PER_METRIC_TON   = 1000;    // number of kgs per metric ton
    private static final int    DAYS_PER_YR          = 365;     // number of days in a calendar year
     ...
     *    DAYS_PER_YR * (kms driven per day * fuel efficiency),
     *    where fuelEfficiency of the car is in liters per km.
     * NOTE: fuel efficiency is input to the method as: liters per 100 km
     *
     * Parameters:  double - fuel efficiency of the car in liters per 100 km,
     *              int - kms driven per day
     *
     * Preconditions: none
     *
    ...
    public static int calcAutoUsage ( double fuelEfficiency, int kmPerDay ) {
        ...
        double kgCO2 = 0;
	double litresPerYear = DAYS_PER_YR * (kmPerDay * (fuelEfficiency/100));
        kgCO2 = KGCO2_PER_LGAS * litresPerYear;
    ...
    public static int calcAutoOwnership ( int Age ) {
        ...
        double kgCO2 = (manufacturing + GJ_MAINT_COST_PER_YR) * M3_PER_GJGAS * KGCO2_PER_M3GAS;
     ...
     * Preconditions: none
     *
     * Returns: carbon foot print of public transit use in kgCO2 cast to an integer
     *
     */
     ...
     * calcCarbonFootprint
     *
     * Purpose: calculates carbon foot print of a person for year as:
    ...
    public static int calcCarbonFootprint ( double[] data ) {
        ...
        double auto_usage = 0;
	double auto_ownership = 0;
	double transit = 0;

    	auto_usage = calcAutoUsage(data[FUEL_EFF_INDEX], (int)data[KMS_DRIVE_INDEX]);
    auto_ownership = calcAutoOwnership((int)data[CAR_AGE_INDEX]);
    transit = calcTransit((int)data[KMS_TRANSIT_INDEX]);
	...
	for(int i=0; i<data.length; i++){
		carbon_footprint_results[i] = calcCarbonFootprint(data[i]);
	}
     ...
     *
     * Parameters: int[] - a 1D array of the carbon footprint result for a population,
     *             int[] - a 1D array to store the stats results (total, max, min and average)
     *
     * Preconditions: none
     *
     * Returns: void
    ...
    public static void populationReport ( int[] carbon_footprints, int[] stats_results ) {

	stats_results[SUM_INDEX] = ArrayOperations.arraySum(carbon_footprints);
	stats_results[MAX_INDEX] = ArrayOperations.arrayMax(carbon_footprints);
	stats_results[MIN_INDEX] = ArrayOperations.arrayMin(carbon_footprints);
	stats_results[AVG_INDEX] = (int)ArrayOperations.arrayAvg(carbon_footprints);
	}


    // HINT:  You should use the methods you wrote in ArrayOperations.java for this method.   T
